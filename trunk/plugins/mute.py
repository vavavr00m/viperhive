	...
	def __init__(self, hub):
        ...
        def onMainChatMsg(self, from_nick, msg):
                ...
                if user.nick in self.mutelist['nicks']:
                        if datetime.datetime.strptime(self.mutelist['nicks'][user.nick]['expired'],'%Y-%m-%dT%H:%M:%S')>datetime.datetime.now():
                ...
                if adr in self.mutelist['addrs']:
                        if datetime.datetime.strptime(self.mutelist['addrs'][adr]['expired'],'%Y-%m-%dT%H:%M:%S')>datetime.datetime.now():
